...
The project remains a local-first MVP and is not ready for real users, but its engineering release gates are now strong: backend/web/mobile validation passes, production images run non-root, dependency audits have no high/critical findings, and the backend/web/proxy release images scan clean at the same threshold. Backend JWT/JWKS validation, OIDC exchange, refresh sessions, revocation, and scoped permission primitives exist and fail closed. The largest remaining blocker is completing actual Apple / Google / Email native callbacks and the end-user permission/session UX, followed by operated cloud database recovery, secret, release-governance, and deployment evidence.
...
| Backend dependencies | Direct and transitive runtime/development packages are pinned through `backend/constraints.txt` and installed under that constraint set in Docker builds. | A live `pip-audit` found 25 advisories across cryptography, JWT, configuration, and HTTP framework packages. | Upgraded to a resolver-verified compatible set: `cryptography 50.0.0`, `pydantic-settings 2.14.2`, `PyJWT 2.13.0`, `FastAPI 0.141.1`, and `Starlette 1.4.1`; the rebuilt image passes migrations, Ruff, strict mypy, 317 tests, and a zero-vulnerability `pip-audit`. | Review Starlette's `httpx` test-client deprecation and migrate to `httpx2` only after compatibility is proven; continue scheduled dependency scans rather than allowing unbounded resolver drift. |
...
| Frontend web | Vite API endpoint and debug flag are environment-driven. | Debug panels can expose transcript-derived data if enabled accidentally; the Transformers browser dependency also pulled vulnerable Node-only transitive packages. | Prior pass hid web debug tools unless `VITE_ENABLE_DEBUG_TOOLS=true`. Lockfile refresh plus fixed `adm-zip` and `sharp` overrides now produce a zero-vulnerability audit while preserving the WebGPU/WASM speech bundle. | Add production UX auth flow once backend auth is real; periodically remove overrides after upstream ranges adopt fixed releases. |
| Mobile | Expo preview talks to backend and has local native model test paths. | Dev client model controls are not end-user production UI; Expo SDK 52's build toolchain had 15 high and one critical dependency findings. | Prior pass hid native dev panel unless `EXPO_PUBLIC_ENABLE_DEBUG_TOOLS=true`; CI runs the full mobile quality gate. Lockfile refresh and reviewed `@xmldom/xmldom`, `postcss`, and `tar` build-tool overrides remove all high/critical audit findings, and CI now enforces that threshold. | Migrate Expo SDK 52 to SDK 57 as a tested breaking change; 16 moderate build-tool advisories remain until then. Add biometric/local DB encryption and production build profiles. |
...
| Dockerfiles | Backend prod runs as non-root app user; web prod uses nginx unprivileged. | Images must remain reproducible and be rebuilt after reviewed dependency changes. | Backend Dockerfiles install through the fully pinned `backend/constraints.txt`; web uses `npm ci` with its lockfile. | Keep image scanning and immutable release tags in the deployment gate. |
| CI/CD | Tests, lint, mypy, secret scan, pip-audit, npm audit, Trivy are present. | The workflow was manual-only, so pushes and pull requests could bypass all release gates; its token also had an unused global `security-events: write` grant. A duplicate test import caused Ruff `F811`, the deterministic year-review test inherited local DeepSeek credentials and made an unmocked network request, strict mypy exposed 163 production/test errors, and mobile dependencies were not scanned. | CI now runs for pushes and pull requests targeting `main` as well as manual dispatch, uses read-only contents permission, and the deployment verifier guards those triggers and permissions. Removed the duplicate import, made year-review tests network-free by default, restored clean strict typing across all 118 checked source files without ignores/exclusions/relaxed rules, and added a mobile high-severity dependency audit. | Configure `main` branch protection to require quality/security jobs, then add cloud deploy workflows with OIDC/workload identity and protected environments. |
