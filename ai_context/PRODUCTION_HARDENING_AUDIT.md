...
| CI/CD | Tests, lint, mypy, secret scan, pip-audit, npm audit, Trivy are present. | The workflow was manual-only, so pushes and pull requests could bypass all release gates; its token also had an unused global `security-events: write` grant. | CI now runs for pushes and pull requests targeting `main` as well as manual dispatch, uses read-only contents permission, and the deployment verifier guards those triggers and permissions. | Configure `main` branch protection to require the quality and security jobs, then add cloud deploy workflows with OIDC/workload identity and protected environments. |
