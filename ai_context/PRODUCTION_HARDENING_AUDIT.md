...
| CI/CD | Tests, lint, mypy, secret scan, pip-audit, npm audit, Trivy are present. | The workflow was manual-only, so pushes and pull requests could bypass all release gates; its token also had an unused global `security-events: write` grant. A duplicate test import caused Ruff `F811`, the deterministic year-review test inherited local DeepSeek credentials and made an unmocked network request, and strict mypy exposed production plus test typing debt. | CI now runs for pushes and pull requests targeting `main` as well as manual dispatch, uses read-only contents permission, and the deployment verifier guards those triggers and permissions. Removed the duplicate import, made year-review tests network-free by default, and restored clean strict typing across all 68 production source files without weakening mypy. | Repair the remaining strict typing in tests, configure `main` branch protection to require quality/security jobs, then add cloud deploy workflows with OIDC/workload identity and protected environments. |
