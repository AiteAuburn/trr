...
Recording preview 與 header busy guard 的 quota exhausted、recording started、reset、finished、too-short 與 busy UI status 必須透過 bounded display helper 產生；錄音秒數只可用於狀態判斷，若進入顯示文案前必須先 clamp。
Record 錄音結果主按鈕必須走 dedicated handler；不可在 JSX Pressable callback 內直接呼叫 `handleRecordingResultPrimaryAction("today")` 或 `handleRecordingResultPrimaryAction("record")`。handler 仍只可切到文字 fallback，不可假造 transcript、呼叫 STT/AI 或寫入紀錄。Home 頁 mic 放開只結束 preview，不顯示錄音結果主按鈕。
...
- 麥克風按鈕置中，為第一視覺重心；下方只顯示淺色提示字：「按住開始說話記錄」與「放開即結束」。按住錄音 preview 時，第二行提示可在同一個 `homeHintSecondary` 位置改顯示「已錄音 X 秒，放開即結束」，不可新增第三行、卡片或文字框。
...
- 首頁按住麥克風只啟動本機錄音 preview 狀態；放開只結束 preview，不自動呼叫 STT / AI / LLM、不中途建立紀錄、不寫入 backend。
...
- record response 寫入 mobile state 前必須 bounded：id/profile_id/record_type/source/occurred_at/metadata/payload_json 欄位都有固定長度、key 數、陣列長度、巢狀深度或數值範圍上限，raw transcript/source_text/raw prompt/raw model output 等 metadata 或 payload key 不保留在正式紀錄 state。
...
- 另提供「原始紀錄」tab 查看語音轉文字內容；raw display item 只可讀取 `metadata_json?.source_text`，有值時必須先經 `boundDisplayText(..., maxDisplayDetailTextLength)`，若正式紀錄 state 沒有 bounded `source_text`，顯示「尚無原始逐字稿；正式紀錄只保留結構化資料。」不可嘗試保留或還原 raw transcript。
...
- 搜尋框必須使用 input-level `maxLength` 與 setter-level 截斷；搜尋只在本機既有預覽兌換項目清單上 filter，不查詢商品 API、不呼叫 AI。
...
- 點數帳本、商品目錄、coupon/discount-code issue 與 coupon use status 已有 backend contract；庫存、購物車、結帳、出貨訂單與付款完成後才可改為完整「查看購物車」或商品兌換流程。
