...
-> 歷史紀錄與基本分析等第一版功能從右上角漢堡選單進入；成就榜、商城、食物社群與其他未來功能先隱藏於一般使用者選單外
-> 使用者說出紀錄內容
-> 系統顯示語音文字稿，使用者可修正
-> 點擊下一步整理
...
- 首頁保留單一大型麥克風操作，不顯示今日摘要、同步狀態、文字輸入、手動新增、紀錄列表、分析 CTA、primary tabs 或 MVP flow stepper。
- 麥克風上方顯示第二版新手引導：主標語「想說什麼就說什麼」左右需有小型提示 icon 強化視覺提示，但不可放大成操作控制；非按鈕提示區顯示「時間 / 血糖 / 飲食 / 運動 / 用藥紀錄 / 身體狀況」，以及帶獨立資訊 icon 的說明 row：「上面這排不是按鈕喔。如果不知道從哪開始，可以參考這些記錄方向；想說什麼就說什麼，不用照固定格式。」提示區必須是淡色背景、無陰影、無凸起感、無 Pressable / button affordance，只作為記錄方向提示；資訊 icon 也不可是 Pressable。
- 引導說明文字需靠近 Icon 區，並與麥克風保持較大距離，讓提示區與操作區明確分開。Icon 不可可點擊；首頁 render block 仍只能有一個 Pressable，也就是大型麥克風。
- 麥克風按鈕仍為第一操作重心；下方顯示淺色提示字：「按住開始說話記錄」與「放開即結束」。按住錄音時，第二行提示可在同一個 `homeHintSecondary` 位置改顯示「已錄音 X 秒，放開即結束」。第二行下方可顯示一行很小的目前語音識別模式：已選本機 Whisper 時顯示「目前語音識別：本機 Whisper」，未選模型時顯示「目前語音識別：內建文字確認」；此文字不可是 Pressable，不可觸發 STT/AI/backend。
- 麥克風下方顯示範例輪播區，標題為「範例（怎麼說都可以）」，自動輪播自然聊天型、簡潔紀錄型、想到什麼說什麼型、生活日記型等說話方式。範例區標題、序號與內文都必須左對齊，並顯示 pagination dots 讓使用者知道目前是哪一個範例；不可是 CTA，不可觸發 AI / STT / backend，也不可讓使用者誤認為必須照固定格式念。
- Mobile navigation verifier 必須從 Today/Home render block 內確認引導標語、非按鈕提示區、說明文字、範例輪播與兩行錄音提示存在，不能只用全檔文字 marker 間接判斷；也必須守住按住時的秒數提示由 bounded helper 產生。Verifier 也必須確認首頁 render block 只有一個 Pressable 麥克風控制，不含 primary/secondary button、section title、status/evidence copy、inline info block 或 record/timeline card surface，且 `homeMicButton` 維持大型圓形尺寸，不可縮成一般 CTA。Today/Home chrome 必須保留空 subtitle 且不可設定 `backTo` 或 `actionLabel` 覆蓋 header fallback；右上角 header button 必須使用 `☰` fallback 開啟功能選單。
- 首頁按住麥克風使用 `expo-av` 啟動本機錄音；放開停止錄音並保留 bounded audio URI。測試版本錄音結束後必須直接進入文字確認 / 語音記錄介面：若已設定 Whisper model path，可轉文字後進入文字確認；若尚未設定 Whisper，也要進入文字確認頁讓使用者補文字。不可直接呼叫 AI / LLM parser、不中途建立紀錄、不寫入 backend。
- 第一版功能入口只保留今日錄音、歷史紀錄、基本分析與必要設定；成就榜、年度回顧、商城、食物社群、社群排行與未來擴充入口不可出現在一般使用者 menu。未來 route 可保留給 debug / visual-smoke，但必須在 `enableDebugTools` 下才可見。
...
- Mobile navigation verifier 必須檢查第一版功能選單 top-level destination id 與使用者可見 label parity，守住「今日錄音」、「歷史紀錄」、「基本分析」、「設定」，並確認成就榜、商城、食物社群等未來功能不在一般 menu 曝光。
...
- History 月曆下方必須顯示完整「每日摘要表」：每個有紀錄日期是一張可點擊 summary card，包含日期、筆數、AI今日摘要、同步狀態、本機/雲端來源與 storage summary。點擊 summary card 只切換選取日期，不呼叫 AI / STT / backend、不寫入資料。
- History 選取日期詳情必須先顯示該日 `AI今日摘要` 與同步/來源標籤，再顯示 AI 整理 / 原始紀錄 mode。AI 整理 mode 不可只列單筆 record card；必須使用每日紀錄相同的垂直 sections：血糖紀錄、飲食紀錄、運動紀錄、體重紀錄、用藥紀錄、其他備註。每個 section 支援多筆，空 section 顯示空白說明，每筆可點進既有 record detail 進行查看/修改。
- History section 欄位需依類型顯示：血糖含測量情境、血糖值、測量時間、備註；飲食含餐別、飲食內容、用餐時間、備註；運動含運動內容、開始時間、運動時長、強度／備註；體重含體重、測量時間、備註；用藥含用藥時間／情境、藥物名稱、劑量、備註；其他備註依 note payload bounded 顯示。未提到的欄位保持空白/未填寫，不強迫共用格式。
...
- Mobile navigation verifier 必須抽取 History render block，確認月曆 grid 在每日摘要表與選取日期詳情 panel 前，且 structured / AI 整理 render branch 在 raw transcript branch 前；點選日期或每日摘要 card 時也必須 reset 到 structured mode。Verifier 也必須抽取 calendar day display helper，確認 `hasRecords` 由該日期 record count 產生、無紀錄日期走 muted style、有紀錄日期顯示 dot/has-records style、selected state 由 selected date key 決定，並守住「亮燈日期有紀錄」legend。Verifier 也必須抽取 raw display helper，守住 `source_text` type guard、bounded display、status label 與 fallback copy。
...
- Android APK script verifier 必須同時守住 README 的 export 說明與 Gradle release bundling contract：debug APK 需要 Metro / `expo start`，release `assembleRelease` 是 standalone APK，不需要 Metro；`mobile/android/app/build.gradle` 必須保留 Expo CLI `bundleCommand = "export:embed"`，確保 release APK 內嵌 JS bundle/assets。
- Android APK prereq checker 必須檢查 root `build.gradle` 的 configured `buildToolsVersion` 在目前執行環境可用；WSL/Linux 跑 Gradle 時不能只看到 Windows SDK 的 `aapt.exe` 就判定 ready，必須要求對應 Linux binary `aapt`，否則 preflight 應先回 blocker，避免 `assembleRelease` 到 `lintVitalReportRelease` 才報 build-tools corrupted。
- Android APK 文件與 prereq recommendation 必須明確區分 `local.properties` 的 SDK path：Windows PowerShell / `gradlew.bat` 需使用 `C:/.../Android/Sdk`；WSL/Linux `./gradlew` 需使用 Linux SDK path，例如 `/home/aite/Android/Sdk`。`/mnt/c/.../Android/Sdk` 只表示 WSL 可讀到 Windows SDK，不代表 Linux Gradle 可使用該 SDK build-tools。
...
- README DeepSeek section 必須列出 Year Review 使用的 system prompt 與 prompt 分析，說明只吃年度聚合統計、只輸出 `important_observation` / `encouragement` JSON、不得收到 raw records / raw transcript / profile id / raw prompt / raw model output，並說明 deterministic fallback。Mobile navigation verifier 必須守住 README 這段文件，避免年度回顧 DeepSeek prompt 只留在 code 而沒有可審核說明。
...
- Backend 已有 `/year-reviews/{year}/share-card/asset` contract，可產生 privacy-masked SVG share-card asset、mime type、filename、alt text 與 checksum；mobile 只可顯示 bounded asset metadata，不可把 raw SVG 直接放入狀態訊息或分享文字。若要分享卡片素材，mobile 必須先把 privacy-masked SVG 寫入本機 cache，並只把安全 file URI 交給原生分享面板。
- Backend 已有 `/year-reviews/{year}/share-card/confirm` contract，必須收到 `privacy_acknowledged=true` 才回傳 share package；未確認隱私遮罩必須 fail-closed。Share package 可標示 `external_share_enabled=true`，mobile 可用 React Native `Share.share` 開啟原生分享面板，分享內容只能使用 privacy-masked `share_text` 與 privacy-masked SVG file URI，不可分享血糖數值、snapshot payload、raw JSON 或未 bounded raw SVG string。
...
- 年度回顧分享整合狀態按鈕會在 backend ready 時請求 privacy-masked SVG share-card asset、確認 share package，將 SVG 寫入本機 cache，並用原生 Share 面板分享 privacy-masked text 與 file URI；visual-smoke 或 backend unavailable 時只能顯示 bounded fallback status，不可啟動外部分享。Mobile navigation verifier 必須守住此 handler 只把 bounded `sharePackage.share_text` 與本機 SVG file URI 傳給 `Share.share`，不可把 `svg_text`、snapshot payload 或 raw JSON 放進分享文字或 UI status。
...
- 食物血糖資料庫定位必須明確顯示「華人使用者真實食物升糖資料庫」與「以實際食用前後血糖分享取代理論與網路傳言」；此為資料產品定位，不代表公開個人健康紀錄，也不可新增未經同意的識別資料或健康資料公開。
...
- 食物列表項目的 accessibility label 必須描述食物資料頁會顯示已載入的分享統計與個別紀錄；不可把一般操作路徑宣稱成「只顯示本機預覽」。
- 食物資料頁顯示：分享總人數、實際升糖參考值（平均）、最高上升血糖、最低上升血糖、個別分享紀錄；平均 / 最高 / 最低上升血糖數值必須顯示 `mg/dL` 單位，列表摘要也必須顯示分享人數與「實際升糖參考值」平均升糖，讓每項食物的實測升糖參考值在 UI 上直接可見。食物血糖變化必須保留 signed delta，若食用後血糖低於食用前，backend 與 mobile 都不可把負值改顯示為 0；個別分享紀錄也必須優先使用 backend `glucose_delta` 並以 signed `mg/dL` 顯示「血糖變化」。個別分享紀錄必須依食用時間由新到舊呈現，避免最新實測資料被舊紀錄壓在後面。Backend integration test 必須直接覆蓋 `/community/foods/{id}` detail response、同食物多筆 share 聚合、個別分享欄位、個別分享排序、負向 `glucose_delta` 聚合，以及不帶 category 的 direct food-name search。Mobile navigation verifier 必須守住四個食物資料頁統計欄位、列表實際升糖參考值摘要、mobile signed delta clamp、個別分享 signed delta 顯示，以及 backend detail shares 的 `eaten_at desc` 排序契約。
...
- 顯示 `每日紀錄` 標題與記錄日期。
- 顯示 `AI今日摘要`，摘要由目前 parser preview records 產生 bounded display copy；新增、編輯或刪除每日紀錄草稿後必須重新整理摘要與分類 sections，並顯示 bounded reorganization revision / reason。此重整不可重新呼叫 LLM、STT 或 backend。
...
- Daily-record draft add / edit / delete 必須走 dedicated reorganization helper，重新 bound preview、重新計算 `AI今日摘要` 與各分類 cards、更新 organization revision / reason，並在摘要卡顯示最新整理狀態。Mobile navigation verifier 必須守住 add / edit / delete 三條 reorganization binding，不可只改 records array 後略過摘要重整狀態。
...
5. Menu screen with grid shortcuts for first version: 今日錄音, 歷史紀錄, 基本分析, 設定. 成就榜、年度回顧、商城、食物社群、社群排行與未來擴充入口只可在 debug / visual-smoke 或未來正式開放時顯示.
