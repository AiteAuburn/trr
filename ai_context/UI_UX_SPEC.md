...
- 麥克風上方顯示第二版新手引導：主標語「想說什麼就說什麼」、非按鈕提示區「時間 / 血糖 / 飲食 / 運動 / 體重 / 身體狀況」，以及說明文字「上面這排不是按鈕喔。如果不知道從哪開始，可以參考這些記錄方向；想說什麼就說什麼，不用照固定格式。」提示區必須是淡色背景、無陰影、無凸起感、無 Pressable / button affordance，只作為記錄方向提示。
...
- 麥克風下方顯示範例輪播區，標題為「範例（怎麼說都可以）」，自動輪播自然聊天型、簡潔紀錄型、想到什麼說什麼型、生活日記型等說話方式。範例區不可是 CTA，不可觸發 AI / STT / backend，也不可讓使用者誤認為必須照固定格式念。
