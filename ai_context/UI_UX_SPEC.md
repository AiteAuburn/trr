...
- 麥克風上方顯示第二版新手引導：主標語「想說什麼就說什麼」左右需有小型提示 icon 強化視覺提示，但不可放大成操作控制；非按鈕提示區顯示「時間 / 血糖 / 飲食 / 運動 / 體重 / 身體狀況」，以及帶獨立資訊 icon 的說明 row：「上面這排不是按鈕喔。如果不知道從哪開始，可以參考這些記錄方向；想說什麼就說什麼，不用照固定格式。」提示區必須是淡色背景、無陰影、無凸起感、無 Pressable / button affordance，只作為記錄方向提示；資訊 icon 也不可是 Pressable。
