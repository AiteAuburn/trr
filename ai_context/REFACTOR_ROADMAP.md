...
- 已存在少量獨立 module：
  ...
  - `sharedDisplayItems.ts`：Shared bounded tuple, metric, option, comparison, tutorial, reminder, session-management preview, and destination-card display item builders.
  ...
  - `settingsChoiceDisplay.ts`：Settings profile and model choice display shaping.
