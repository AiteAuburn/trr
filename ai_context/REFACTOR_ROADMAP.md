...
- 已存在少量獨立 module：
  ...
  - `settingsCopy.ts`：Settings navigation, active-profile display copy, voice quota minute formatting/warning/usage copy, model/quota/reminder/privacy bounded copy.
  - `subscriptionCopy.ts`：Subscription plan/status labels, account security, profile boundary, and settings subscription section labels / bounded copy.
