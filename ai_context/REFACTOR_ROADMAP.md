...
- 已存在少量獨立 module：
  ...
  - `firstVersionFlowCopy.ts`：Today / Record quick-entry display items and summary copy, core first-version flow labels/accessibility copy, shared header/recording accessibility copy, shared busy/clear action status, save-success / AI-save-failure navigation status, and first-version flow status messages.
