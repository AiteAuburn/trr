...
- 已存在少量獨立 module：
  ...
  - `recordEditTransforms.ts`：record edit field type, max-length bounds, empty edit fields, and record payload-to-edit-field transforms.
...
Status: started; Settings row, Analysis range, History detail-mode, History calendar day display, shared record-list display, record detail display, manual-record confirmation display, and record edit-field transform boundaries extracted.
