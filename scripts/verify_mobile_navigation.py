...
def main() -> int:
    ...
    try:
        ...
        for label, marker in (
            ...
            ("profile settings display items helper binding", "const profileChoiceDisplayItems = useMemo(\n    () => settingsProfileChoiceDisplayItems(profiles),"),
            ...
            ("food community point rows helper binding", "const foodCommunityPointRows = foodCommunityPointDisplayRows(storePointsBalance);"),
            ...
            ("food community item display items helper binding", "const foodCommunityDisplayItems = useMemo(\n    () => buildFoodCommunityItemDisplayItems(foodCommunityItemsForDisplay),"),
            ("food community visible helper binding", "const visibleFoodCommunityItems = visibleFoodCommunityDisplayItems("),
            ("food community selected helper binding", "const selectedFoodCommunityItem = selectedFoodCommunityDisplayItem("),
            ...
            ("food community share field rows helper binding", "const foodCommunityShareFieldRows = foodCommunityShareFieldDisplayRows("),
            ("food community point rows helper binding", "const foodCommunityPointRows = foodCommunityPointDisplayRows(storePointsBalance);"),
            ("food community ranking rows helper binding", "const foodCommunityRankingRows = foodCommunityRankingDisplayRows();"),
        ...
        for label, marker in (
            ...
            ("store category display items helper binding", "const storeCategoryDisplayOptions = useMemo(() => buildStoreCategoryDisplayItems(storeCategories), []);"),
            ("store product display items helper binding", "const storeProductDisplayItems = useMemo(\n    () => buildStoreProductDisplayItems(storeProductsForDisplay),"),
            ("store visible helper binding", "const visibleStoreProducts = visibleStoreProductDisplayItems("),
            ("food community category display items helper binding", "const foodCommunityCategoryDisplayOptions = useMemo(\n    () => buildFoodCommunityCategoryDisplayItems(foodCommunityCategoriesForDisplay),"),
            ...
            ("store redemption wallet helper binding", "const storeRedemptionDisplayItems = useMemo(\n    () => storeRedemptionWalletDisplayItems(storeRedemptions),"),
            ("store redemption boundary rows helper binding", "const storeRedemptionBoundaryRows = storeRedemptionBoundaryDisplayRows("),
            ...
            ("year review yearly records helper binding", "const yearlyRecords = useMemo(\n    () => recordsInYear(records, yearReviewTargetDisplayYear),"),
            ("year review type counts helper binding", "const yearlyTypeCounts = useMemo(() => recordTypeCounts(yearlyRecords), [yearlyRecords]);"),
            ("year review glucose records helper binding", "const yearlyGlucoseRecords = buildAnalysisGlucoseRecords(yearlyRecords);"),
            ("year review glucose values helper binding", "const yearlyGlucoseValues = buildAnalysisGlucoseValues(yearlyGlucoseRecords);"),
            ("year review highest glucose helper binding", "const yearlyGlucoseHighest = highestNumber(yearlyGlucoseValues);"),
            ("year review lowest glucose helper binding", "const yearlyGlucoseLowest = lowestNumber(yearlyGlucoseValues);"),
            ("year review record day count helper binding", "const yearlyRecordDayCount = recordDayCount(yearlyRecords);"),
            ...
            ("year review glucose count helper binding", "const yearlyGlucoseRecordDisplayCount = clampNumber(yearlyGlucoseRecords.length, 0, maxMobileCountValue);"),
