...
def main() -> int:
    ...
    try:
        ...
        for label, marker in (
            ...
            ("manual record type selector label helper fields", "return type.label;"),
            ("manual record type selector selected helper", "function manualRecordTypeOptionSelected(type: ManualRecordTypeOption, selectedValue: string)"),
            ("manual record type selector selected helper fields", "return selectedValue === manualRecordTypeOptionKey(type);"),
            ("manual record type selector selected binding", "const typeSelected = manualRecordTypeOptionSelected(type, selectedValue);"),
            ("manual record type selector key binding", "key={manualRecordTypeOptionKey(type)}"),
            ("manual record type selector accessibility", "accessibilityLabel={manualRecordTypeOptionAccessibilityLabel(type)}"),
            ("manual record type selector role", 'accessibilityRole="button"'),
            ("manual record type selector selected state", "accessibilityState={{ selected: typeSelected }}"),
            ("manual record type selector press", "onPress={() => onTypePress(type)}"),
            ("manual record type selector label", "{manualRecordTypeOptionLabel(type)}"),
        ...
        for label, marker in (
            ...
            ("manual record meal option key helper", "function manualRecordMealOptionKey(option: ManualRecordMealOption)"),
            ("manual record meal option key helper fields", "return option.value;"),
            ("manual record meal option accessibility helper", "function manualRecordMealOptionAccessibilityLabel(option: ManualRecordMealOption)"),
            ("manual record meal option accessibility helper fields", "return option.accessibilityLabel;"),
            ("manual record meal option label helper", "function manualRecordMealOptionLabel(option: ManualRecordMealOption)"),
            ("manual record meal option label helper fields", "return option.label;"),
            ("manual record meal option selected helper", "function manualRecordMealOptionSelected(option: ManualRecordMealOption, mealType: string)"),
            ("manual record meal option selected helper fields", "return mealType === manualRecordMealOptionKey(option);"),
            ("manual record meal type options", "mealTypeOptions.map((option) => {"),
            ("manual record meal selected binding", "const optionSelected = manualRecordMealOptionSelected(option, mealType);"),
            ("manual record meal type key", "key={manualRecordMealOptionKey(option)}"),
            ("manual record meal type accessibility", "accessibilityLabel={manualRecordMealOptionAccessibilityLabel(option)}"),
        ...
        _assert_contains(
            "AI candidate edit selection clear helper",
            content,
        ...
        _assert_contains(
            "record update request blocked helper",
            content,
            "function isRecordUpdateRequestBlocked()",
        )
        _assert_contains(
            "record update request blocked helper internals",
            content,
        ...
        _assert_contains(
            "record delete request blocked helper",
            content,
            "function isRecordDeleteRequestBlocked()",
        )
        _assert_contains(
            "record delete request blocked helper internals",
            content,
        ...
        for label, marker in (
            ...
            ("parse request body helper", "function parserPreviewRequestBody("),
            ("parse request body helper internals", "const modelIds = parserPreviewModelIds();\n    return {\n      profile_id: profileId,\n      transcript: text,\n      stt_model_id: modelIds.sttModelId,\n      llm_model_id: modelIds.llmModelId,\n      occurred_at: occurredAt,\n      voice_seconds: voiceSeconds"),
            ("parse request model ids helper", "function parserPreviewModelIds()"),
            ("parse request model ids helper internals", "return {\n      sttModelId,\n      llmModelId\n    };"),
            ("parse request helper", "function requestParserPreview(\n    accountId: string,\n    profileId: string,"),
            ("parse request helper endpoint", 'return requestJson<ParsePreviewResponse>(\n      normalizedApiBaseUrl,\n      "/ai/parse-preview",'),
            ("parse request options helper binding", "parserPreviewRequestOptions(accountId, profileId, text, occurredAt, voiceSeconds)"),
            ("parse request options helper", "function parserPreviewRequestOptions(\n    accountId: string,\n    profileId: string,"),
            ("parse request helper headers", "headers: protectedRequestHeaders(accountId, accessToken),"),
            ("parse request options helper method", 'method: "POST",'),
            ...
            ("parse prepared request helper", "function requestPreparedParserPreview("),
            ("parse prepared request helper internals", "return requestMergedParserPreviewForContext(\n      context,\n      text,\n      preparedRequest.parseOccurredAt,\n      preparedRequest.parserVoiceSeconds,\n      preparedRequest.existingDailyPreview"),
            ("parse submit prepared helper", "async function submitPreparedParserPreview("),
            ("parse submit prepared helper internals", "const mergedDailyPreview = await requestPreparedParserPreview(context, text, preparedRequest);\n    handlePreparedParserPreviewSuccess(mergedDailyPreview, text, preparedRequest);"),
            ("parse complete prepared helper", "async function completePreparedParserPreviewRequest("),
            ("parse complete prepared helper internals", "try {\n      await submitPreparedParserPreview(context, text, preparedRequest);\n    } catch (error) {\n      handleParserPreviewFailure(error);\n    } finally {\n      finishParserPreviewRequest();"),
            ("parse start and complete helper", "async function startAndCompleteParserPreviewRequest(context: {"),
            ("parse start and complete helper internals", "const preparedRequest = startPreparedParserPreviewRequest();\n    await completePreparedParserPreviewRequest(context, parserTranscriptText(), preparedRequest);"),
            ("parse request helper binding", "await startAndCompleteParserPreviewRequest(parserContext);"),
            ("parse request voice seconds", "voice_seconds: voiceSeconds"),
            ("parse request voice seconds binding", "preparedRequest.parserVoiceSeconds,\n      preparedRequest.existingDailyPreview\n    );"),
            ...
            ("parse merge response helper binding", "const mergedDailyPreview = await requestPreparedParserPreview(context, text, preparedRequest);"),
            ...
            ("parse prepared success helper", "function handlePreparedParserPreviewSuccess("),
            ("parse prepared success helper internals", "handleParserPreviewSuccess(\n      nextPreview,\n      preparedRequest.parseOccurredAt,\n      text,\n      preparedRequest.parserVoiceSeconds"),
            ("parse success helper binding", "handlePreparedParserPreviewSuccess(mergedDailyPreview, text, preparedRequest);"),
            ("parse transcript append helper", "function appendParserTranscriptEntry(occurredAt: string, text: string, voiceSeconds: number)"),
            ("parse transcript append helper internals", "const transcriptEntry = createDailyTranscriptEntry(\n      occurredAt,\n      text,\n      parserTranscriptSource(voiceSeconds)\n    );\n    if (transcriptEntry) {\n      appendDailyTranscriptEntry(transcriptEntry);"),
            ("parse transcript append helper binding", "appendParserTranscriptEntry(occurredAt, text, voiceSeconds);"),
            ("parse success clears voice seconds helper", "function resetParserTranscriptVoiceSeconds() {\n    setTranscriptVoiceSeconds(0);"),
            ("parse success clears voice seconds helper binding", "function openAiReviewAfterParserSuccess() {\n    resetParserTranscriptVoiceSeconds();"),
            ...
            ("parse success refresh quota helper internals", "const currentAccount = account;\n    if (shouldRefreshVoiceQuotaAfterParserSuccess(voiceSeconds, currentAccount)) {\n      void loadVoiceQuota(currentAccount.id);"),
            ("parse success refresh quota guard helper", "function shouldRefreshVoiceQuotaAfterParserSuccess("),
            ("parse success refresh quota guard helper internals", "return voiceSeconds > 0 && Boolean(currentAccount);"),
            ("parse success refreshes quota", "refreshVoiceQuotaAfterParserSuccess(voiceSeconds);"),
            ...
            ("parse transcript validation message helper", "function parserTranscriptValidationMessage()"),
            ("parse transcript validation message helper internals", "return transcriptValidationError;"),
            ("parse transcript validation error binding", "const validationMessage = parserTranscriptValidationMessage();\n    if (validationMessage) {\n      handleParserTranscriptValidationError(validationMessage);"),
            ("parse transcript text helper", "function parserTranscriptText()"),
            ("parse transcript text helper internals", "return transcript;"),
            ("parse transcript text helper binding", "completePreparedParserPreviewRequest(context, parserTranscriptText(), preparedRequest);"),
            ...
            ("parse existing daily preview helper internals", "return preview;"),
            ("parse voice seconds helper", "function parserPreviewVoiceSeconds()"),
        ...
        for label, marker in (
            ("daily record save confirm title binding", "{aiSaveConfirmTitleDisplayText}"),
            ("daily record save confirm date label binding", "{aiSaveConfirmDateLabelDisplayText}"),
            ("daily record summary title binding", "{aiSaveConfirmSummaryLabelDisplayText}"),
            ("daily record transcript title binding", "{todayTranscriptTitleDisplayText}"),
            ...
            ("daily record parse occurred timestamp", "parseOccurredAt: parserPreviewOccurredAt()"),
            ("daily record parse sends shared timestamp", "preparedRequest.parseOccurredAt,\n      preparedRequest.parserVoiceSeconds,\n      preparedRequest.existingDailyPreview"),
            ...
            ("daily record reorganization next revision helper", "function nextDailyRecordOrganizationRevision()"),
            ("daily record reorganization next revision helper internals", "function nextDailyRecordOrganizationRevision() {\n    return clampNumber(dailyRecordOrganizationRevision + 1, 0, maxMobileCountValue);"),
            ("daily record reorganization next revision binding", "const nextRevision = nextDailyRecordOrganizationRevision();"),
            ("daily record reorganization status for change helper", "function dailyRecordReorganizationStatusForChange("),
            ("daily record reorganization status for change helper internals", "return statusOverride ??\n      dailyRecordReorganizationStatusMessage(reason, reorganizedPreview.records.length, nextRevision);"),
            ("daily record reorganization status for change binding", "setStatus(\n      dailyRecordReorganizationStatusForChange(reason, reorganizedPreview, nextRevision, statusOverride)\n    );"),
            ...
            ("daily record save start helper binding", "startPreviewSaveRequest();"),
            ("daily record save finish helper", "function finishPreviewSaveRequest()"),
            ...
            ("daily record save guarded context helper internals", "if (!preview || previewState.isEmpty) {\n      return null;\n    }\n    if (!protectedBackendReady)"),
            ("daily record save guarded context account", "return {\n      account,\n      preview\n    };"),
            ("daily record save guarded context submit binding", "const saveContext = guardedDailyRecordSaveContext();\n    if (!saveContext) {\n      return;\n    }"),
            ("daily record save submit helper", "async function submitDailyRecordSave(saveContext: { account: Account; preview: ParsePreviewResponse })"),
            ("daily record save submit helper internals", "const recordsToSave = buildPreviewRecordsForCurrentSave(saveContext.preview);\n    const saveResponse = await requestDailyRecordSave(saveContext.account.id, saveContext.preview, recordsToSave);\n    handleDailyRecordSaveSuccess(saveResponse, recordsToSave.length);"),
            ("daily record save complete helper", "async function completeDailyRecordSaveRequest(saveContext: { account: Account; preview: ParsePreviewResponse })"),
            ("daily record save complete helper internals", "try {\n      await submitDailyRecordSave(saveContext);\n    } catch (error) {\n      handleDailyRecordSaveFailure(error);\n    } finally {\n      finishPreviewSaveRequest();"),
            ("daily record save complete helper binding", "await completeDailyRecordSaveRequest(saveContext);"),
            ("daily record save start and complete helper", "async function startAndCompleteDailyRecordSaveRequest(saveContext: { account: Account; preview: ParsePreviewResponse })"),
            ("daily record save start and complete helper internals", "startPreviewSaveRequest();\n    await completeDailyRecordSaveRequest(saveContext);"),
            ("daily record save start and complete helper binding", "await startAndCompleteDailyRecordSaveRequest(saveContext);"),
            ("daily record save submit helper binding", "await submitDailyRecordSave(saveContext);"),
            ("daily record save records helper binding", "const recordsToSave = buildPreviewRecordsForCurrentSave(saveContext.preview);"),
            ...
            ("daily record save request body helper", "function dailyRecordSaveRequestBody(nextPreview: ParsePreviewResponse, recordsToSave: PendingRecord[])"),
            ("daily record save request body helper internals", "return buildDailyRecordSaveRequest(nextPreview, recordsToSave, dailyTranscriptEntries);"),
            ("daily record save request helper payload", "body: JSON.stringify(dailyRecordSaveRequestBody(nextPreview, recordsToSave))"),
            ("daily record save request helper binding", "const saveResponse = await requestDailyRecordSave(saveContext.account.id, saveContext.preview, recordsToSave);"),
            ...
            ("daily record save created records helper", "function dailyRecordSaveCreatedRecords(saveResponse: DailyRecordSaveResponse)"),
            ("daily record save created records helper internals", "return boundRecordsList(saveResponse.records, maxMobilePreviewRecords);"),
            ("daily record save success helper records", "const createdRecords = dailyRecordSaveCreatedRecords(saveResponse);"),
            ("daily record save draft clear helper", "function clearDailyRecordSaveDraftState()"),
        ...
        for label, marker in (
            ...
            ("daily record save confirm direct title label", "<Text style={styles.sectionTitle}>每日紀錄</Text>"),
            ("daily record save confirm direct date label", "<Text style={styles.confidence}>記錄日期</Text>"),
            ("daily record save confirm direct summary label", "<Text style={styles.previewModeBadge}>AI今日摘要</Text>"),
            ("daily record transcript direct title label", "<Text style={styles.label}>今日錄音文字</Text>"),
            ("daily record transcript direct body label", "<Text style={styles.evidence}>保留今天所有文字片段；目前先顯示本次整理內容。</Text>"),
        ...
        for label, marker in (
            ...
            ("direct daily record entry menu index condition", "dailyRecordMenuIndex === item.index ? ("),
            ("direct daily record entry edit accessibility binding", "accessibilityLabel={item.editAccessibilityLabel}"),
            ("direct daily record entry edit label literal", "<Text style={styles.secondaryButtonText}>編輯</Text>"),
        ...
        for label, marker in (
            ...
            ("daily record transcript display bundle title", "title: boundDisplayText(\"今日錄音文字\", maxDisplayTextLength)"),
            ("daily record transcript display bundle body", "body: boundDisplayText(\"保留今天所有文字片段；目前先顯示本次整理內容。\", maxDisplayDetailTextLength)"),
        ...
        for label, marker in (
            ...
            ("preview record state low confidence count binding", "const lowConfidenceRecordCount = records.filter((record) => (record.confidence ?? 1) < 0.7).length;"),
            ("preview record state rejected event count binding", "const rejectedEventCount = rejectedEvents.length;"),
            ("preview record state warning flag", "hasWarnings: lowConfidenceRecordCount > 0 || rejectedEventCount > 0"),
            ...
            ("preview warning state binding", "const hasAiSaveConfirmWarnings = previewState.hasWarnings;"),
            ...
            ("preview action daily return state binding", 'const isPreviewActionReturningToDailyRecord = previewActionReturnScreen === "aiSaveConfirm";'),
            ("daily record remove confirm return state binding", "const isDailyRecordRemoveConfirm = isPreviewActionReturningToDailyRecord;"),
        ...
        for label, marker in (
            ...
            ("direct preview warning count binding", "lowConfidencePreviewRecordCount > 0 || rejectedPreviewEventCount > 0"),
            ...
            ("direct daily record remove confirm return screen binding", 'const isDailyRecordRemoveConfirm = previewActionReturnScreen === "aiSaveConfirm";'),
            ("direct preview action daily return branch", 'if (previewActionReturnScreen === "aiSaveConfirm") {'),
        ...
        for label, marker in (
            ...
            ("load more guarded context helper", "function guardedMoreRecordSyncContext()"),
            ("load more guarded context visual smoke", "if (visualSmokePreviewActive.current) {\n      setRecordsStatus(visualSmokeRecordSyncStatusMessage());\n      return null;"),
            ("load more guarded context unavailable", "if (!protectedBackendReady) {\n      setRecordsStatus(recordSyncUnavailableStatusMessage(protectedBackendUnavailableMessage));\n      return null;"),
            ("load more guarded context key", "syncKey: `${normalizedApiBaseUrl}:${account.id}:${activeProfileId}:before:${cursorRecord.occurred_at}:${cursorRecord.created_at}`"),
            ("load more guarded context binding", "const syncContext = guardedMoreRecordSyncContext();\n    if (!syncContext) {\n      return;\n    }"),
            ("load more guarded context in-flight binding", "recordSyncInFlightKeys.current.has(syncContext.syncKey)"),
            ("load more start helper", "function startMoreRecordSyncRequest(syncContext: { syncKey: string })"),
            ("load more start helper internals", "if (recordSyncInFlightKeys.current.has(syncContext.syncKey)) {\n      return false;\n    }\n    recordSyncInFlightKeys.current.add(syncContext.syncKey);\n    setRecordsStatus(recordSyncPageLoadingStatusMessage());\n    return true;"),
            ("load more start helper binding", "if (!startMoreRecordSyncRequest(syncContext))"),
            ("load more request helper", "function requestMoreRecordSync(syncContext: {\n    account: Account;\n    activeProfileId: string;\n    cursorRecord: RecordItem;"),
            ("load more guarded context request profile binding", "profile_id: syncContext.activeProfileId"),
            ("load more request helper endpoint", "`/records?${query.toString()}`"),
            ("load more guarded context request account binding", "headers: protectedRequestHeaders(syncContext.account.id, accessToken)"),
            ("load more request helper binding", "const response = await requestMoreRecordSync(syncContext);"),
            ("load more success helper", "function handleMoreRecordSyncSuccess(response: RecordItem[])"),
            ("load more success helper internals", "const boundedPage = boundRecordsList(response, mobileRecordSyncLimit);\n    setRecords((current) => mergeRecordsByCursorOrder(current, boundedPage));\n    const hasMoreAfterPage = response.length >= mobileRecordSyncLimit;"),
            ("load more success status helper binding", "recordSyncPageSuccessStatusMessage(\n        nextCount,\n        boundedPage.length,\n        maxMobileRecordCacheLimit,\n        hasMoreAfterPage && nextCount < maxMobileRecordCacheLimit"),
            ("load more success helper binding", "handleMoreRecordSyncSuccess(response);"),
            ("load more failure helper", "function handleMoreRecordSyncFailure()"),
            ("load more failure helper internals", "setRecordsStatus(recordSyncFailureStatusMessage());"),
            ("load more failure helper binding", "handleMoreRecordSyncFailure();"),
            ("load more finish helper", "function finishMoreRecordSyncRequest(syncContext: { syncKey: string })"),
            ("load more finish helper internals", "recordSyncInFlightKeys.current.delete(syncContext.syncKey);"),
            ("load more finish helper binding", "finishMoreRecordSyncRequest(syncContext);"),
            ("load more complete helper", "async function completeMoreRecordSyncRequest(syncContext: {\n    account: Account;\n    activeProfileId: string;\n    cursorRecord: RecordItem;\n    syncKey: string;"),
            ("load more complete helper internals", "try {\n      const response = await requestMoreRecordSync(syncContext);\n      handleMoreRecordSyncSuccess(response);\n    } catch {\n      handleMoreRecordSyncFailure();\n    } finally {\n      finishMoreRecordSyncRequest(syncContext);"),
            ("load more complete helper binding", "await completeMoreRecordSyncRequest(syncContext);"),
            ("load more start and complete helper", "async function startAndCompleteMoreRecordSyncRequest(syncContext: {\n    account: Account;\n    activeProfileId: string;\n    cursorRecord: RecordItem;\n    syncKey: string;"),
            ("load more start and complete helper internals", "if (!startMoreRecordSyncRequest(syncContext)) {\n      return;\n    }\n    await completeMoreRecordSyncRequest(syncContext);"),
            ("load more start and complete helper binding", "await startAndCompleteMoreRecordSyncRequest(syncContext);"),
            ...
            ("initial record sync guarded context helper", "function guardedInitialRecordSyncContext()"),
            ("initial record sync guarded context visual smoke", "if (visualSmokePreviewActive.current) {\n      setRecordsStatus(visualSmokeRecordSyncStatusMessage());\n      return null;"),
            ("initial record sync guarded context unavailable", "if (!protectedBackendReady) {\n      setRecordsStatus(recordSyncUnavailableStatusMessage(protectedBackendUnavailableMessage));\n      return null;"),
            ("initial record sync guarded context key", "syncKey: `${normalizedApiBaseUrl}:${account.id}:${activeProfileId}`"),
            ("initial record sync guarded context binding", "const syncContext = guardedInitialRecordSyncContext();\n    if (!syncContext) {\n      return;\n    }"),
            ("initial record sync start helper", "function startInitialRecordSyncRequest(syncContext: { syncKey: string })"),
            ("initial record sync start helper internals", "latestRecordSyncKey.current = syncContext.syncKey;\n    if (recordSyncInFlightKeys.current.has(syncContext.syncKey)) {\n      return false;\n    }\n    recordSyncInFlightKeys.current.add(syncContext.syncKey);\n    setRecordsStatus(recordSyncLoadingStatusMessage());\n    return true;"),
            ("initial record sync start helper binding", "if (!startInitialRecordSyncRequest(syncContext))"),
            ("initial record sync latest key binding", "latestRecordSyncKey.current = syncContext.syncKey;"),
            ("initial record sync request helper", "function requestInitialRecordSync(syncContext: { account: Account; activeProfileId: string })"),
            ("initial record sync request profile binding", "profile_id: syncContext.activeProfileId"),
            ("initial record sync request endpoint binding", "`/records?${query.toString()}`"),
            ("initial record sync request account binding", "headers: protectedRequestHeaders(syncContext.account.id, accessToken)"),
            ("initial record sync request helper binding", "const response = await requestInitialRecordSync(syncContext);"),
            ("initial record sync success helper", "function handleInitialRecordSyncSuccess(response: RecordItem[])"),
            ("initial record sync success helper internals", "const boundedResponse = boundRecordsList(response, mobileRecordSyncLimit);\n    setRecords(boundedResponse);\n    setRecordsHasMore(response.length >= mobileRecordSyncLimit);"),
            ("initial record sync success status helper binding", "recordSyncSuccessStatusMessage(\n        boundedResponse.length,\n        mobileRecordSyncDisplayLimit,\n        maxMobileRecordCacheLimit,\n        response.length >= mobileRecordSyncLimit"),
            ("initial record sync success helper binding", "handleInitialRecordSyncSuccess(response);"),
            ("initial record sync failure helper", "function handleInitialRecordSyncFailure(syncContext: { syncKey: string })"),
            ("initial record sync failure helper internals", "if (latestRecordSyncKey.current === syncContext.syncKey) {\n      clearRecordSyncPaginationStatus(recordSyncFailureStatusMessage());"),
            ("initial record sync failure helper binding", "handleInitialRecordSyncFailure(syncContext);"),
            ("initial record sync finish helper", "function finishInitialRecordSyncRequest(syncContext: { syncKey: string })"),
            ("initial record sync finish helper internals", "recordSyncInFlightKeys.current.delete(syncContext.syncKey);"),
            ("initial record sync finish helper binding", "finishInitialRecordSyncRequest(syncContext);"),
            ("initial record sync complete helper", "async function completeInitialRecordSyncRequest(syncContext: {\n    account: Account;\n    activeProfileId: string;\n    syncKey: string;"),
            ("initial record sync complete helper internals", "try {\n      const response = await requestInitialRecordSync(syncContext);\n      if (latestRecordSyncKey.current !== syncContext.syncKey) {\n        return;\n      }\n      handleInitialRecordSyncSuccess(response);\n    } catch {\n      handleInitialRecordSyncFailure(syncContext);\n    } finally {\n      finishInitialRecordSyncRequest(syncContext);"),
            ("initial record sync complete helper binding", "await completeInitialRecordSyncRequest(syncContext);"),
        ...
        for label, marker in (
            ...
            ("history cursor before query", "before: syncContext.cursorRecord.occurred_at,"),
            ("history cursor created_at query", "before_created_at: syncContext.cursorRecord.created_at"),
        ...
        for label, marker in (
            ...
            ("manual meal selected state", "accessibilityState={{ selected: optionSelected }}"),
            ("preview glucose unit selected state", 'const optionSelected = editOptionIsSelected(option, recordEditFieldValue(previewEditFields, "glucoseUnit"));'),
            ("preview glucose timing selected state", 'const optionSelected = editOptionIsSelected(option, recordEditFieldValue(previewEditFields, "glucoseTiming"));'),
            ("preview meal selected state", 'const optionSelected = editOptionIsSelected(option, recordEditFieldValue(previewEditFields, "mealType"));'),
            ...
            ("record edit glucose timing selected state", 'const optionSelected = editOptionIsSelected(option, recordEditFieldValue(recordEditFields, "glucoseTiming"));'),
            ("record edit meal selected state", 'const optionSelected = editOptionIsSelected(option, recordEditFieldValue(recordEditFields, "mealType"));'),
        ...
        for label, marker in (
            ...
            ("preview edit glucose unit selected helper binding", 'editOptionIsSelected(option, recordEditFieldValue(previewEditFields, "glucoseUnit"))'),
            ("preview edit glucose timing selected helper binding", 'editOptionIsSelected(option, recordEditFieldValue(previewEditFields, "glucoseTiming"))'),
            ("preview edit meal type selected helper binding", 'editOptionIsSelected(option, recordEditFieldValue(previewEditFields, "mealType"))'),
            ...
            ("record edit glucose timing selected helper binding", 'editOptionIsSelected(option, recordEditFieldValue(recordEditFields, "glucoseTiming"))'),
            ("record edit meal type selected helper binding", 'editOptionIsSelected(option, recordEditFieldValue(recordEditFields, "mealType"))'),
        ...
        for label, marker in (
            ...
            ("downloaded whisper models helper", "function downloadedWhisperModels(models: ReturnType<typeof boundDownloadedModels>)"),
            ("downloaded whisper models helper fields", 'return models.filter((model) => model.kind === "whisper" && model.exists);'),
            ...
            ("native debug action blocked helper fields", "return isBusy;"),
            ("native debug action blocked helper binding", "if (isNativeDebugActionBlocked()) {\n      return false;\n    }"),
            ("native debug action start helper", "function startNativeDebugAction()"),
            ("native debug action start helper fields", "setIsBusy(true);"),
            ("native debug action start helper binding", "startNativeDebugAction();"),
            ("native debug action finish helper", "function finishNativeDebugAction()"),
            ("native debug action finish helper fields", "setIsBusy(false);"),
            ("native debug action finish helper binding", "finishNativeDebugAction();"),
            ("native debug action ready helper", "function isNativeDebugActionReady()"),
            ("native debug action ready helper fields", "if (isNativeDebugActionBlocked()) {\n      return false;\n    }\n    if (!enableDebugTools) {\n      openNativeDebugUnavailable();\n      return false;\n    }\n    return true;"),
            ...
            ("native debug action ready benchmark binding", "if (!isNativeDebugActionReady()) {\n      return;\n    }\n    startNativeDebugAction();\n    startNativeBenchmarkStatus();"),
            ...
            ("native whisper request args helper", "function nativeWhisperRequestArgs(whisperInput: ReturnType<typeof nativeWhisperInput>)"),
            ("native whisper request args helper fields", "return {\n      modelPath: whisperInput.modelPath,\n      audioPath: whisperInput.audioPath\n    };"),
            ("native whisper input predicate helper", "function hasNativeWhisperInput(whisperInput: ReturnType<typeof nativeWhisperInput>)"),
            ("native whisper input predicate helper fields", "return Boolean(whisperInput.modelPath && whisperInput.audioPath);"),
            ("native whisper input guard binding", "if (!hasNativeWhisperInput(whisperInput))"),
            ("native whisper call input binding", "const text = await transcribeWithNativeWhisper(nativeWhisperRequestArgs(whisperInput));"),
            ("native llama input helper", "function nativeLlamaInput()"),
            ("native llama input helper fields", "modelPath: llamaModelPath.trim(),\n      transcript: transcript.trim()"),
            ("native llama input helper binding", "const llamaInput = nativeLlamaInput();"),
            ("native llama request args helper", "function nativeLlamaRequestArgs(llamaInput: ReturnType<typeof nativeLlamaInput>)"),
            ("native llama request args helper fields", "return {\n      modelPath: llamaInput.modelPath,\n      transcript: llamaInput.transcript\n    };"),
            ("native llama input predicate helper", "function hasNativeLlamaInput(llamaInput: ReturnType<typeof nativeLlamaInput>)"),
            ("native llama input predicate helper fields", "return Boolean(llamaInput.modelPath && llamaInput.transcript);"),
            ("native llama input guard binding", "if (!hasNativeLlamaInput(llamaInput))"),
            ("native llama call input binding", "const output = await parseWithNativeLlama(nativeLlamaRequestArgs(llamaInput));"),
            ("downloaded models refresh failure helper", "function handleDownloadedModelsRefreshFailure(error: unknown, showStatus: boolean)"),
            ("downloaded models refresh failure helper fields", "setNativeStatus(nativeDownloadedModelsFailureStatusMessage(error));\n    if (showStatus) {\n      setStatus(recordingModelRefreshFailureStatusMessage(error));\n    }"),
            ("downloaded models refresh failure helper binding", "handleDownloadedModelsRefreshFailure(error, showStatus);"),
            ("native whisper success helper", "function handleNativeWhisperSuccess(text: string)"),
            ("native whisper success helper fields", "updateTranscriptDraft(text);\n    setNativeStatus(nativeWhisperSuccessStatusMessage());"),
            ("native whisper success helper binding", "handleNativeWhisperSuccess(text);"),
            ("native whisper start status helper", "function startNativeWhisperStatus()"),
            ("native whisper start status helper fields", "setNativeStatus(nativeWhisperProgressStatusMessage());"),
            ("native whisper start status helper binding", "startNativeWhisperStatus();"),
            ("native whisper missing input helper", "function handleNativeWhisperMissingInput()"),
            ("native whisper missing input helper fields", "setNativeStatus(nativeWhisperMissingInputStatusMessage());"),
            ("native whisper missing input helper binding", "handleNativeWhisperMissingInput();"),
            ("native llama success helper", "function handleNativeLlamaSuccess(output: string)"),
            ("native llama success helper fields", "setLlamaDebugOutput(nativeLlamaOutputSummaryMessage(output.length));\n    setNativeStatus(nativeLlamaSuccessStatusMessage());"),
            ("native llama success helper binding", "handleNativeLlamaSuccess(output);"),
            ("native llama start status helper", "function startNativeLlamaStatus()"),
            ("native llama start status helper fields", "setNativeStatus(nativeLlamaProgressStatusMessage());"),
            ("native llama start status helper binding", "startNativeLlamaStatus();"),
            ("native llama missing input helper", "function handleNativeLlamaMissingInput()"),
            ("native llama missing input helper fields", "setNativeStatus(nativeLlamaMissingInputStatusMessage());"),
            ("native llama missing input helper binding", "handleNativeLlamaMissingInput();"),
            ("native benchmark missing input helper", "function handleNativeBenchmarkMissingInput()"),
            ("native benchmark missing input helper fields", "setNativeStatus(nativeBenchmarkMissingInputStatusMessage());"),
            ("native benchmark missing input helper binding", "handleNativeBenchmarkMissingInput();"),
            ("native benchmark start status helper", "function startNativeBenchmarkStatus()"),
            ("native benchmark start status helper fields", "setNativeStatus(nativeBenchmarkProgressStatusMessage());"),
            ("native benchmark start status helper binding", "startNativeBenchmarkStatus();"),
            ("native benchmark result alias", "type NativeBenchmarkResult = Awaited<ReturnType<typeof benchmarkNativeWhisper>> | Awaited<ReturnType<typeof benchmarkNativeLlama>>;"),
            ("native benchmark success helper", "function handleNativeBenchmarkSuccess(results: NativeBenchmarkResult[])"),
            ("native benchmark success helper fields", "setNativeStatus(nativeBenchmarkResultStatusMessage(results));"),
            ("native benchmark success helper binding", "handleNativeBenchmarkSuccess(results);"),
            ("native whisper benchmark append helper", "async function appendNativeWhisperBenchmarkResult("),
            ("native whisper benchmark append helper result alias", "results: NativeBenchmarkResult[],\n    whisperInput: ReturnType<typeof nativeWhisperInput>"),
            ("native whisper benchmark append helper fields", "if (hasNativeWhisperInput(whisperInput)) {\n      results.push(await benchmarkNativeWhisper(nativeWhisperRequestArgs(whisperInput)));\n    }"),
            ("native whisper benchmark append helper binding", "await appendNativeWhisperBenchmarkResult(results, whisperInput);"),
            ("native llama benchmark append helper", "async function appendNativeLlamaBenchmarkResult("),
            ("native llama benchmark append helper result alias", "results: NativeBenchmarkResult[],\n    llamaInput: ReturnType<typeof nativeLlamaInput>"),
            ("native llama benchmark append helper fields", "if (hasNativeLlamaInput(llamaInput)) {\n      results.push(await benchmarkNativeLlama(nativeLlamaRequestArgs(llamaInput)));\n    }"),
            ("native llama benchmark append helper binding", "await appendNativeLlamaBenchmarkResult(results, llamaInput);"),
            ("native benchmark results alias binding", "const results: NativeBenchmarkResult[] = [];"),
            ("native benchmark results helper", "async function nativeBenchmarkResults()"),
            ("native benchmark results helper fields", "const results: NativeBenchmarkResult[] = [];\n    const whisperInput = nativeWhisperInput();\n    await appendNativeWhisperBenchmarkResult(results, whisperInput);\n    const llamaInput = nativeLlamaInput();\n    await appendNativeLlamaBenchmarkResult(results, llamaInput);\n    return results;"),
            ...
            ("native model download request args helper", "function nativeModelDownloadRequestArgs()"),
            ("native model download request args helper fields", "return {\n      url: modelUrl,\n      kind: downloadKind,\n      onProgress: setDownloadProgress\n    };"),
            ("native model download request args helper binding", "const uri = await downloadModel(nativeModelDownloadRequestArgs());"),
            ("native model download failure helper", "function handleNativeModelDownloadFailure(error: unknown)"),
            ("native model download failure helper fields", "setNativeStatus(nativeModelDownloadFailureStatusMessage(error));"),
            ("native model download failure helper binding", "handleNativeModelDownloadFailure(error);"),
            ("native module check success helper", "function handleNativeModuleCheckSuccess(message: string)"),
            ("native module check success helper fields", "setNativeStatus(nativeModuleCheckResultStatusMessage(message));"),
            ("native module check success helper binding", "handleNativeModuleCheckSuccess(result.message);"),
            ("native module check start status helper", "function startNativeModuleCheckStatus()"),
            ("native module check start status helper fields", "setNativeStatus(nativeModuleCheckProgressStatusMessage());"),
            ("native module check start status helper binding", "startNativeModuleCheckStatus();"),
            ("native module check failure helper", "function handleNativeModuleCheckFailure(error: unknown)"),
            ("native module check failure helper fields", "setNativeStatus(nativeModuleCheckFailureStatusMessage(error));"),
            ("native module check failure helper binding", "handleNativeModuleCheckFailure(error);"),
            ("native whisper failure helper", "function handleNativeWhisperFailure(error: unknown)"),
            ("native whisper failure helper fields", "setNativeStatus(nativeWhisperFailureStatusMessage(error));"),
            ("native whisper failure helper binding", "handleNativeWhisperFailure(error);"),
            ("native llama failure helper", "function handleNativeLlamaFailure(error: unknown)"),
            ("native llama failure helper fields", "setNativeStatus(nativeLlamaFailureStatusMessage(error));"),
            ("native llama failure helper binding", "handleNativeLlamaFailure(error);"),
            ("native model download success helper", "async function handleNativeModelDownloadSuccess(uri: string)"),
